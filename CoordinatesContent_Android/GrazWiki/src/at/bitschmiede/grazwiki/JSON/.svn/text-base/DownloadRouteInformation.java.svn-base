...
public class DownloadRouteInformation extends AsyncTask<Integer, Integer, Void> {

	protected Vector<DownloadEvents> _listeners;
	private int _routeID;
	private RouteInformation _routeInfoList = null;
	...
	protected Void doInBackground(Integer... params) {
		...
		_routeInfoList = null;
		try {
			_routeInfoList =  myRestService.getRouteInformation(_routeID);
	...
	protected void onPostExecute(Void result) {
		...
		if (_listeners != null && !_listeners.isEmpty())
		{
			...
			while (e.hasMoreElements())
			{
				...
				eventHandler.downloadRouteInformationCompleted(_routeID, _routeInfoList);
			}
		}
	}
